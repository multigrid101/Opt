...
if exParser._doTimeAll:
    for homedir in folders:
        print("exp0026.py: performing simulations for {0}".format(homedir))
        doTimingsExp000234(homedir, 'JTJ', 'backend_cuda', 1)
...
if exParser._doPlotAll:
    ...
    counter=0
    for homedir in folders:
        print("exp0026.py: creating plot for {0}".format(homedir))

        # first save the individual pdfplot
        makePlotExp000234(homedir, 'JTJ', 'backend_cuda')

        makePlotExp000234(homedir, 'JTJ', 'backend_cuda', axarr=axarr, index=counter)
        counter += 1

    finalfig.legend(ncol=5, loc='lower center',
            fontsize=7, bbox_to_anchor=(0.5,0.02))
    finalfig.suptitle('Opt-gpu jtj', fontsize=15, y=0.92)
    finalfig.savefig(open("./timings/exp0026_singlepage.pdf", "wb"),
            bbox_inches='tight',
            format='pdf')

    hlp.mergePdfsOfAvailableFiles("./timings/exp0027.pdf", folders, "{0}/timings/exp0026.pdf")
...
if exParser._exampleName is not None:
    ...
    print("exp0026.py: performing simulations for {0}".format(homedir))
    doTimingsExp000234(exParser._exampleName, 'JTJ', 'backend_cuda', 1)

    print("exp0026.py: creating plot for {0}".format(homedir))
    makePlotExp000234(exParser._exampleName, 'JTJ', 'backend_cuda')

    hlp.mergePdfsOfAvailableFiles("./timings/exp0026.pdf", folders, "{0}/timings/exp0026.pdf")
