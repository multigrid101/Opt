...
if exParser._doTimeAll:
    for homedir in folders:
        print("exp0015.py: performing simulations for {0}".format(homedir))
        doTimingsExp0013(homedir, 2)
...
if exParser._doPlotAll:
    ...
    counter=0
    for homedir in folders:
        print("exp0015.py: creating plot for {0}".format(homedir))

        # first save the individual pdfplot
        makePlotExp0013(homedir, 15)

        # add plot to single-page plot
        makePlotExp0013(homedir, 15, fig=finalfig, axarr=axarr, index=counter)
        counter += 1

    finalfig.legend(ncol=3, loc='lower center',
            fontsize=7, bbox_to_anchor=(0.45,0.01))
    finalfig.suptitle('Opt-mt(n) strong scaling large size', fontsize=15, y=0.92)
    finalfig.savefig(open("./timings/exp0015_singlepage.pdf", "wb"),
            bbox_inches='tight',
            format='pdf')
    ...
    for homedir in folders:
        ...
        merger.append(open("{0}/timings/exp0015.pdf".format(homedir), "rb"))
    merger.write("./timings/exp0015.pdf")
...
if exParser._exampleName is not None:
    ...
    print("exp0015.py: performing simulations for {0}".format(homedir))
    doTimingsExp0013(exParser._exampleName, 2)

    print("exp0015.py: creating plot for {0}".format(homedir))
    makePlotExp0013(exParser._exampleName, 15)
