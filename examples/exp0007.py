...
if exParser._doTimeAll:
    for homedir in folders:
        print("exp0007.py: performing simulations for {0}".format(homedir))
        doTimingsExp000234(homedir, 'JTJ', 'backend_cpu_mt', 1)
...
if exParser._doPlotAll:
    ...
    counter=0
    for homedir in folders:
        print("exp0007.py: creating plot for {0}".format(homedir))
    ...
    finalfig.savefig(open("./timings/exp0007_singlepage.pdf", "wb"),
            bbox_inches='tight',
            format='pdf')
    ...
    for homedir in folders:
        ...
        merger.append(open("{0}/timings/exp0007.pdf".format(homedir), "rb"))
    merger.write("./timings/exp0007.pdf")
...
if exParser._exampleName is not None:
    ...
    print("exp0007.py: performing simulations for {0}".format(homedir))
    doTimingsExp000234(exParser._exampleName, 'JTJ', 'backend_cpu_mt', 1)

    print("exp0007.py: creating plot for {0}".format(homedir))
    makePlotExp000234(exParser._exampleName, 'JTJ', 'backend_cpu_mt')
