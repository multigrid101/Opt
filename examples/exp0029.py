...
from myPlots import makePlotExp0029
from myTimings import doTimingsExp0029
from myArgParsers import experimentParser
from myHelpers import mergePdfsOfAvailableFiles
...
if exParser._doTimeAll:
    for homedir in folders:
        print("exp0029.py: performing simulations for {0}".format(homedir))
        doTimingsExp0029(homedir)
...
if exParser._doPlotAll:
    ...
    counter=0
    for homedir in folders:
        print("exp0029.py: creating plot for {0}".format(homedir))

        # first save the individual pdfplot
        if os.path.isfile("{0}/timings/exp0029.timing".format(homedir)):
            makePlotExp0029(homedir)

            makePlotExp0029(homedir, axarr=axarr, index=counter)
            counter += 1

    finalfig.legend(ncol=3, loc='lower center',
            fontsize=7, bbox_to_anchor=(0.5,0.02))
    finalfig.suptitle('Opt-mt(4)-fusedjtj vs. Opt-cuda-matfree vs. Opt-cuda-fusedjtj', fontsize=15, y=0.92)
    finalfig.savefig(open("./timings/exp0029_singlepage.pdf", "wb"),
            bbox_inches='tight',
            format='pdf')

    hlp.mergePdfsOfAvailableFiles("./timings/exp0029.pdf", folders, "{0}/timings/exp0029.pdf")
